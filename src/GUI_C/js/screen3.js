...
app.controller('screen3Ctrl', function($scope, $http) {
    ...
    var httpGet = function () {
        ...
        $http.get(prefix+'t_qualityTest_oneTimeQualified').success(function(response) {
            $scope.qualityTest.oneTimeQualified = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });

        $http.get(prefix+'t_qualityTest_repairQulified').success(function(response) {
            $scope.qualityTest.repairQulified = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });
        ...
        $http.get(prefix+'t_scheduleInfo_successRate').success(function(response) {
            $scope.scheduleInfo.successRate = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });

        $http.get(prefix+'t_scheduleInfo_conflictRate').success(function(response) {
            $scope.scheduleInfo.conflictRate = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });

        $http.get(prefix+'t_scheduleInfo_faliure').success(function(response) {
            $scope.scheduleInfo.faliure = JSON.parse(response.data).content.count;
        });

        // 供应链
        $http.get(prefix+'t_supplyChain_materials').success(function(response) {
            $scope.supplyChain.materials = JSON.parse(response.data).content.count;
        });

        $http.get(prefix+'t_supplyChain_accessories').success(function(response) {
            $scope.supplyChain.accessories = JSON.parse(response.data).content.count;
        });

        $http.get(prefix+'t_supplyChain_parts').success(function(response) {
            $scope.supplyChain.parts = JSON.parse(response.data).content.count;
        });

        $http.get(prefix+'t_supplyChain_completeRate').success(function(response) {
            $scope.supplyChain.completeRate = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });

        $http.get(prefix+'t_supplyChain_intimeRate').success(function(response) {
            $scope.supplyChain.intimeRate = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });
        ...
        var env_img_html = "";
        for(key in envList){
            env_img_html += "<div class='col-md-6 img_padding'>"
                 + "<img src='" + envList[key].img + "' alt='' width='100%' height='200px'/>"
                 + "</div>";
        } 
        document.getElementById('env_img').innerHTML = env_img_html;

        var process_bar = $scope.scheduleFinalList;
        var process_bar_html = "";
        for(key in process_bar){
            process_bar_html += "<div class='progress' style='width:" + process_bar[key].num + "'>"
                 + "<span class='schedule_bar'>" + process_bar[key].type + "</span>"
                 + "</div>";
        } 
        document.getElementById('process_bar').innerHTML = process_bar_html;
