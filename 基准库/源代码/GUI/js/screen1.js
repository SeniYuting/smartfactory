...
app.controller('screen1Ctrl', function($scope, $http) {
    ...
    var httpGet = function () {
        ...
        $http.get(prefix+'t_today_order').success(function(response) {
            $scope.order.todayOrder = JSON.parse(response.data).content.count;
        });
        $http.get(prefix+'t_order_amount').success(function(response) {
            $scope.order.orderAmount = (JSON.parse(response.data).content.sum/10000).toFixed(2); //单位换算成万
        });
        $http.get(prefix+'t_today_schedule').success(function(response) {
            $scope.order.todaySchedule = JSON.parse(response.data).content.count;
        });
        $http.get(prefix+'t_order_promptness').success(function(response) {
            $scope.order.promptness = (parseFloat(JSON.parse(response.data).content.value)*100).toFixed(1)+'%';
        });
        $http.get(prefix+'t_order_emergency').success(function(response) {
            $scope.order.emergency = JSON.parse(response.data).content.count;
        });
        $http.get(prefix+'t_order_variation').success(function(response) {
            $scope.order.variation = JSON.parse(response.data).content.count;
        });
        $http.get(prefix+'t_order_exception').success(function(response) {
            $scope.order.exception = JSON.parse(response.data).content.count;
        });

        // rule {}
        // $http.get(prefix+'t_customservice_worksheet').success(function(response) { 
        //     $scope.customService.workSheet = JSON.parse(response.data).content.count;
        // });

        // rule exp
        $http.get(prefix+'t_customservice_worksheet_add').success(function(response) {
            $scope.customService.workSheet = JSON.parse(response.data).content.result;
        });

        $http.get(prefix+'t_customservice_online').success(function(response) {
            $scope.customService.online = JSON.parse(response.data).content.online;
        });
        $http.get(prefix+'t_customservice_total').success(function(response) {
            $scope.customService.total = JSON.parse(response.data).content.total;
        });
